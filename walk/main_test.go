...
func createTempDir(t *testing.T, files map[string]int) (dirname string, cleanup func()) {
	...
	for k, n := range files {
		for j := 1; j <= n; j++ {
			...
			if err := os.WriteFile(fpath, []byte("sample"), 0644); err != nil {
				t.Fatal(err)
			}
		}
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(lines) != expLogLines {
				t.Errorf("expected %d log lines, got %d", expLogLines, len(lines))
			}
			...
			if len(filesLeft) != tc.nNoDelete {
				t.Errorf("expected %d file left, got %d", tc.nNoDelete, len(filesLeft))
			}
		})
	}
}
